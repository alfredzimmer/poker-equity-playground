...
export default function CardSelectorGrid({
  usedCards,
  onCardSelect,
}: CardSelectorGridProps) {
          ...
          return (
            ...
            >
              <div className="flex flex-col items-center justify-center h-full">
                <span
                  className={`text-[8px] leading-none ${!used ? "text-slate-600 dark:text-slate-400" : "text-slate-400 dark:text-slate-600"}`}
                >
                  {rank}
                </span>
                <span
                  className={`text-base leading-none ${!used ? suitColors[suit] : "text-slate-400 dark:text-slate-600"}`}
                >
                  {suitSymbols[suit]}
                </span>
              </div>
